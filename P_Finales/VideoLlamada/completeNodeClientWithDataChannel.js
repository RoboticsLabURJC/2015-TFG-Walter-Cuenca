...
/*** WebRTC data structures(streams and node_peer)  ***/
var localStream;
var remoteStream;
var pc;
...
function sendData() {
  //añadimos el mensaje a nuestra ventana
  var line = document.createElement('li');
  var textnode = document.createTextNode("Walter:"+sendTextarea.value);
  line.appendChild(textnode);   
  document.getElementById("chat").appendChild(line);
  //componemos el mensaje para enviar al otro Peer 
  var data = JSON.stringify({info:'chat',data:sendTextarea.value});
  if(isInitiator){
...
//dataChannel.send(arrayBufferToBase64(date_file.slice(start, 100)));
// Handlers...
...
function handleMessage(event) {
  ...
  if(_data.info == 'chat'){
    ...
    var line = document.createElement('li');
    var textnode = document.createTextNode('Luna:'+_data.data);
    line.appendChild(textnode);   
    document.getElementById("chat").appendChild(line);
    receiveTextarea.value += _data.data + '\n';
    trace('Received message:'+ _data.data);
    ...
    if(_data.end != undefined){
      ...
      console.log(blob);
        ...
        document.getElementById("list_file").appendChild(link)
      }else{
        console.log(blob);
        var link = document.createElement('a');
        link.href = 'data:'+_data.more.type+';base64'+blob;
